  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("503")) {
        errorText =
          "The LLaMA API service is currently unavailable. This could be due to a configuration issue with pydantic. Please check with your administrator.";
      } else if (
        error instanceof Error &&
        error.message.includes("Failed to fetch")
      ) {
        errorText =
          "Unable to connect to the API server. Please check your network connection or server status.";
      }
