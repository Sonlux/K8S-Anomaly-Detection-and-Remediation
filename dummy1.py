...
class KubernetesAgenticRAG:
    ...
    def detect_minikube_environment(self):
        ...
        try:
            ...
            # Fallback methods if k8s_knowledge_fetcher is not available
            # Method 1: Check environment variables
            if os.environ.get("MINIKUBE_ACTIVE_DOCKERD") or os.environ.get("MINIKUBE_HOME"):
                return True
                
            # Method 2: Try to run minikube status command
            import subprocess
            try:
            ...
            # Method 3: Check if PROMETHEUS_URL is set to localhost:9090 (common for Minikube setups)
            if os.environ.get("PROMETHEUS_URL") == "http://localhost:9090":
                return True
    ...
    def query(self, user_input: str) -> str:
        ...
        k8s_info = None
        if self._is_k8s_info_query(user_input) and k8s_fetcher_available and hasattr(self, 'is_minikube') and self.is_minikube:
            try:
                logger.info("Fetching real-time Kubernetes information from Minikube...")
                k8s_info = fetch_k8s_info(user_input)
                if self.debug:
                    logger.debug(f"Retrieved Kubernetes information: {k8s_info[:200]}...")
            except Exception as e:
                logger.error(f"Error fetching Kubernetes information: {e}")
